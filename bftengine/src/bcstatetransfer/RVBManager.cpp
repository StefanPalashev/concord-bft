...
void RVBManager::init(bool fetching) {
  ...
  LOG_FATAL(GL, "__ss__ :" << KVLOG(rvbDataOnInit));
...
void RVBManager::updateRvbDataDuringCheckpoint(CheckpointDesc& new_checkpoint_desc) {
  ...
  if (new_checkpoint_desc.rvbData.empty()) {
    LOG_FATAL(GL, "__ss__ empty RVB DATA!");
    ...
    c1.writeDigest(buff);
    Digest digest(buff);
    LOG_FATAL(GL, "__ss__ digest on update BEFORE potential pruning: " << KVLOG(digest));
    LOG_FATAL(GL, "__ss__ digest (buff) on update BEFORE potential pruning: " << KVLOG(buff));
    //
  ...
  if (!in_mem_rvt_->empty()) {
    ...
    if (config_.enableStoreRvbDataDuringCheckpointing) {
      ...
      const auto rvbDataRestored =
          concordUtils::bufferToHex(new_checkpoint_desc.rvbData.data(), new_checkpoint_desc.rvbData.size());
      LOG_FATAL(GL, "__ss__ :" << KVLOG(rvbDataRestored));

    } else {
      new_checkpoint_desc.rvbData.clear();
      LOG_FATAL(GL, "__ss__ rvbCleared");
    }
