...
void ReplicaImp::onMessage<CheckpointMsg>(CheckpointMsg *msg) {
  ...
  LOG_INFO(GL,
           "Received checkpoint message from node. " << KVLOG(msgSenderId,
                                                              ...
                                                              otherDigest,
                                                              rvbDataDigest));
  LOG_WARN(GL, "__sscp__" << KVLOG(msgSenderId, msgGenReplicaId, msgSeqNum, rvbDataDigest));
...
void ReplicaImp::finalizePPExecution(PrePrepareMsg *ppMsg) {
  ...
  if (lastExecutedSeqNum % checkpointWindowSize == 0) {
    ...
    LOG_INFO(GL, "about to get digest" << KVLOG(addressof(rvbDataDigest), rvbDataDigest));
    stateTransfer->getDigestOfCheckpoint(
        checkpointNum, sizeof(Digest), state, (char *)&stateDigest, (char *)&otherDigest, (char *)&rvbDataDigest);

    LOG_INFO(GL, "got digest" << KVLOG(addressof(rvbDataDigest), rvbDataDigest));
